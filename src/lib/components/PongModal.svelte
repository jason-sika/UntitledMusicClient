<script>
  let { onClose, onSent, pingId, senderUsername = "" } = $props();

  const MAX_PONG_LENGTH = 240;
  ...
  async function sendPong() {
    if (sending || !pingId) return;
    error = "";
    success = "";
    sending = true;

    try {
      const res = await fetch("https://backend.umc.jasonsika.com/api/pong", {
        method: "POST",
        credentials: "include",
      ...
      if (!res.ok) {
        error = data?.error || "Could not send pong.";
      } else {
        success = "Ponged back!";
        onSent?.();
...
<div class="flowbox">
  <div class="pingwrapper">
    <div class="rim card" class:errorS={error} class:successS={success}>
      ...
      <p class="hint">Send a reply back — with a message, or just a pong.</p>

      <div class="formInput rim">
        <textarea
          placeholder="Message (optional)..."
          maxlength={MAX_PONG_LENGTH}
          bind:value={message}
          rows="3"></textarea>
      </div>
      <p class="charcount">{message.length}/{MAX_PONG_LENGTH}</p>
